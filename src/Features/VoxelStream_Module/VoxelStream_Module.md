...
Pipeline phân tích mức độ tập trung qua khuôn mặt thời gian thực sử dụng webcam và MediaPipe Face Landmarker, kết hợp xác thực khuôn mặt bằng DeepFace (ArcFace).
...
Capture → Detect → Extract → HeadPoseEstimator → FSM → FocusAnalyzer → Render → Display
         ...
         FaceAuthenticator ── Đăng ký khuôn mặt (phím S)
                ↓
         Xác thực định kỳ mỗi 90 frames
                ↓
         Cảnh báo nếu không khớp khuôn mặt
```
...
```
GIAI ĐOẠN 1 – ĐĂNG KÝ KHUÔN MẶT:
  Hiển thị hướng dẫn "Nhìn thẳng camera, nhấn S để đăng ký"
  Nhấn S → FaceAuthenticator.register_face(frame) → lưu embedding ArcFace
  Nhấn Q → thoát

GIAI ĐOẠN 2 – GIÁM SÁT (mỗi frame):
  ...
  XÁC THỰC ĐỊNH KỲ:
  Cứ 90 frames (~3s): FaceAuthenticator.verify_face(frame)
    → Nếu không khớp: hiển thị cảnh báo đỏ "WARNING: UNAUTHORIZED USER!"
```
...
│   ├── FaceAuth.py                 # Xác thực khuôn mặt (DeepFace ArcFace + cosine distance)
...
```
GIAI ĐOẠN ĐĂNG KÝ:
  Hiển thị hướng dẫn → chờ phím S → FaceAuthenticator.register_face(frame)
  Nếu thành công → is_registered = True → vào vòng giám sát
  Nếu thất bại → thông báo lỗi → chờ thử lại

VÒNG GIÁM SÁT (mỗi frame):
  detector.detect(frame) → nếu có mặt:
    extractor.extract(landmarks) → metrics (ear, mar)
    head_pose_estimator.estimate(tf_matrix) → pitch, yaw, roll
    fsm.update(metrics) → state
    renderer.render(frame, landmarks, state, metrics)
    if session_manager: session_manager.update(state)
  nếu không có mặt:
    fsm.update(None) → state
    renderer.render_no_face(frame, state)

  XÁC THỰC ĐỊNH KỲ (mỗi 90 frames):
    FaceAuthenticator.verify_face(frame)
    → Nếu sai: auth_warning = True → vẽ cảnh báo đỏ
    → Nếu đúng: auth_warning = False
...
### FaceAuthenticator (`services/FaceAuth.py`)

Xác thực khuôn mặt người dùng bằng **DeepFace** với model **ArcFace** và khoảng cách **cosine**:

| Phương thức       | Mô tả                                           |
| ----------------- | ----------------------------------------------- |
| `register_face()` | Trích xuất embedding ArcFace từ khung hình, lưu |
| `verify_face()`   | So sánh embedding hiện tại với embedding đã đăng ký |

**Ngưỡng** (`threshold = 0.68`): khoảng cách cosine dưới 0.68 được coi là khớp.

**Luồng hoạt động:**
1. Khi mới chạy, chương trình yêu cầu nhấn **S** để đăng ký khuôn mặt
2. Sau khi đăng ký, mỗi **90 frames (~3s)** kiểm tra lại khuôn mặt
3. Nếu không khớp → hiển thị cảnh báo `WARNING: UNAUTHORIZED USER!`
...
- `fastapi` — HTTP API
- `granian` — ASGI server
- `deepface` — Xác thực khuôn mặt (ArcFace)
- `scipy` — Khoảng cách cosine cho so sánh embedding
